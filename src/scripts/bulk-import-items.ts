...
function parseUrls(field: string): string[] {
	if (!field || field.trim() === "") return [];
...
function parseListingPrice(price: string): string {
	if (!price || price.trim() === "") return "0";
	return price.replace(/^KWD\s*/i, "").replace(/,/g, "").trim();
}

// Parse year
function parseYear(year: string): string | undefined {
	if (!year || year.trim() === "") return undefined;
	return year.trim();
}
...
const itemsData = [
	...
	},
	{
		...
		seller: "Faisal Al-Abdulhadi",
		priceEstimator:
			"image.png (https://v5.airtableusercontent.com/v3/u/50/50/1770127200000/by8CiMdZcmhrQzUu6l8w3g/MkiOIrJ7MlZKyADqmHSJHJZ6YpM1z-BCN3KsfF2lb3Y175Z30kP0NbaQmlN-Ly8WNEDSeSLPBPtF6L1Z-PUe9dkBxtVxKlI5u7yRx8pW02MOgjvL4YLLgw7_rnefZ-AZJaUb-b9LPtxMe3dcyIvBtQ/qFL0h5Xr5Cb_do6wIJTagEbpKxuJ4vmy58ma4CCn-1k)",
		quote:
	...
	},
	{
		...
		year: "2014",
		photo:
			"e92e70fe-910f-4982-9d8e-db4c5d7a8bf9.jpeg (https://v5.airtableusercontent.com/v3/u/50/50/1770127200000/Ld9jkF--MzrnqCskuMrzPw/Hb4PpF2CTptQwTs2cZgjdT57CDVA_y3k3fP-m-VLt0EeHmXHP2Qt2q0GTmMzsr6Sepm3R9vrCH4N-JcqhTEBtOQgLrHC0cvQZvL8r53ybxdFP26jWKr_nl-EPkqoNJ6-2xVwvOoGBlMBj-oIZsfPBTIZYRrBaNPbRXGuFxuC5bGN3JXuTa3Y8HyrIbZ8r64S/qqvkn77x-LKJbGNZYogyX9DLLw31fTieU2AcEptF_Dg)",
		receiptPhoto: "",
...
async function bulkImportItems() {
	try {
		...
		let defaultCategory = await Category.findOne({ name: "Bags" });
		if (!defaultCategory) {
		...
		for (const itemData of itemsData) {
			try {
				...
				let seller = null;
				let sellerUserId: mongoose.Types.ObjectId | undefined = undefined;
				...
				if (sellerEmail && sellerEmail.trim() !== "") {
					try {
						...
						if (sellerUser) {
							seller = await Seller.findOne({ userId: sellerUser._id }).session(session);
							sellerUserId = sellerUser._id as mongoose.Types.ObjectId;
						}
					} catch (error) {
					...
					[
						{
							...
							condition: ItemCondition.LikeNew, // Default
							uploadedAt: new Date(),
							saleRate: "0", // Default
							itemStatus: normalizeBoolean(itemData.approved) ? ItemStatus.Approved : ItemStatus.Pending,
				...
				if (seller) {
					await Seller.findByIdAndUpdate(
						seller._id,
						{ $addToSet: { itemIds: newItem[0]._id } },
						{ session },
					);
				}
				...
				results.success.push({
					...
					itemId: (newItem[0]._id as mongoose.Types.ObjectId).toString(),
				});
