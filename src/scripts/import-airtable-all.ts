...
function parseAddress(raw: string): {
  street: string; city: string; block: string; governorate: string; house: string; flat?: string;
} {
  const s = (raw || "").trim();
  ...
  const city =
    s.split(",")[0]?.trim().split(/\s+/)[0]?.trim() || "Kuwait City";
 ...
 */
async function importSellerUsers(): Promise<Map<string, mongoose.Types.ObjectId>> {
  ...
  for (const r of rows) {
    ...
    const existing = await User.findOne({ emailAddress: safeEmail, isDeleted: false });
    if (existing) {
...
async function importBuyerUsers(): Promise<number> {
  ...
  for (const r of rows) {
    ...
    if (await User.findOne({ emailAddress: safeEmail, isDeleted: false })) continue;
